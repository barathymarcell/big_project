			<form id="signin_teacher" class="form-signin" method="post">
					<h3 class="form-signin-heading"><i class="icon-lock"></i> Regisztrálj oktatóként!</h3>
					<input type="text" class="input-block-level"  name="firstname" placeholder="Vezetéknév" required>
					<input type="text" class="input-block-level"  name="lastname" placeholder="Keresztnév" required>
					<label>Tanterület</label>
					...
					<button id="signin" name="login" class="btn btn-info" type="submit"><i class="icon-check icon-large"></i> Regisztrálás</button>
			</form>
			<script>
			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Üdvözlünk a GitBarmok LMS Felületén!", { header: 'Sikeres bejelentkezés!' });
						var delay = 1000;
							setTimeout(function(){ window.location = 'dasboard_teacher.php'  }, delay);  
						}else{
							$.jGrowl("Az adataid nincsenek tárolva az adatbázisban!", { header: 'Sikertelen regisztráció!' });
						}
						}
					});
