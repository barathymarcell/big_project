...
if ($filedesc == '') {
    $errmsg_arr[] = 'állomány leírása hiányzik.';
    $errflag = true;
}

if ($_FILES['uploaded_file']['size'] >= 1048576 * 5) {
    $errmsg_arr[] = 'a kiválaszott állomány meghaladja az 5MB méretet.';
    $errflag = true;
}
...
if ((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
    ...
    if (($ext != "exe") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload")) {
		...
		$name_notification  = 'Adja hozzá a letöltési elem nevét: '." ".'<b>'.$name.'</b>';
